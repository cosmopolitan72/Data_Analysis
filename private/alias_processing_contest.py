                ...
                while True:
                        ...
                        while True: 
                            ...
                            if Suppresion_Type in ['2', " 부분 삭제", "2. 부분 삭제"]:
                                ...
                                def partial_delete_column(alias_df):
                                    ...
                                    alias_df[column_name] = alias_df[column_name].astype(str).str.slice(0, cut_index)
                                    print(f"'{column_name}' 부분 삭제되었습니다.")
                                    print(alias_df[[column_name]].head())
                            ...
                            if Suppresion_Type in ['5', "마스킹", "5. 마스킹"]:
                                ...
                                def masking_column(alias_df):
                                    ...
                                    def mask_data(data):
                                        data = str(data)
                                        if len(data) < front_keep + back_keep:
                                            # 데이터가 너무 짧으면 그대로 반환
                                            return data
                                        front = data[:front_keep]
                                        back = data[-back_keep:] if back_keep > 0 else ''
                                        masked_len = len(data) - front_keep - back_keep
                                        masked = masking_char * masked_len
                                        return front + masked + back

                                    alias_df[target_col] = alias_df[target_col].apply(mask_data)

                                    print(f"\n마스킹된 '{target_col}' 컬럼으로 변경되었습니다.")
                                    print(alias_df[[target_col]].head())
                                    return
                                ...
                                alias_df = masking_column(alias_df)
                                save_file(alias_df)
                                return alias_df
                    ...
                    if Type_Of_Program in ['3', "일반화 기술", "3. 일반화 기술"]:
                        ...
                        def Generalization_(alias_df):
                            while True:
                                generalization_tools = input("0. 뒤로가기, 1. 라운딩, 2. 상하단코딩, 3. 로컬 일반화, 4. 범위방법, 5. 문자데이터 범주화, 6. 단어 끼리 묶기 중에 고르시오\n>")
                                ...
                                if generalization_tools in ['6', '단어끼리 묶기', '6. 단어끼리 묶기']:
                                    print("1. 가명처리 - 3. 일반화 기술 - 6. 단어끼리 묶기 선택하셨습니다.")

                                    def text_bounding(alias_df):
                                        target_col = input("단어끼리 묶을 컬럼명을 입력하세요 (예: 승차 Top1 행정동) > ")
                                        ...
                                        def extract_words(alias_df):
                                            words = input("묶을 단어들을 입력하세요\n>").split(" ")
                                            words = [w.strip() for w in words]

                                            new_words = input("묶은 데이터 명을 입력하세요\n>")

                                            alias_df.loc[alias_df[target_col].isin(words), target_col] = new_words
                                            print(f"{words}가 {new_words}로 변경되었습니다.")
                                        extract_words(alias_df)

                                    text_bounding(alias_df)
                                    save_file(alias_df)
                                    return
